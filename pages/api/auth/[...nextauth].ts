...
function getMainDomainUrl(): string {
  ...
  return process.env.NEXTAUTH_URL || "https://dealroom.open-mic.co.za";
}
  ...
  providers: [
    ...
    EmailProvider({
      async sendVerificationRequest({ identifier, url, provider }) {
        // Rate limit: max 3 requests per email per 10 minutes (using Upstash Redis)
        const { success } = await ratelimit(3, "10 m").limit(
          `auth:email:${identifier.trim().toLowerCase()}`,
        );
        if (!success) {
  ...
  cookies: {
    sessionToken: {
      ...
      options: {
        ...
        sameSite: "lax",
        path: "/",
        // Keep the cookie host-only so authentication works on custom domains.
        domain: undefined,
