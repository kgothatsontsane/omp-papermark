...
| F14 | Data capture gap: prod /api/record_view rejected (page_views__v3 empty) | FIXED | Vercel had TINYBIRD_TOKEN but NO TINYBIRD_URL → zod-bird defaulted baseUrl to global api.tinybird.co (wrong region) → reject/rate-limit. Added TINYBIRD_URL=https://api.eu-west-1.aws.tinybird.co to Vercel (id H2RBFuu1YDEkqIEK), redeployed. VERIFIED: prod /api/record_view with real Chrome UA header → 200 + row in page_views__v3 with browser=Chrome 126, os=Mac OS 10.15.7, device=Desktop, country=ZA, city=Johannesburg (Vercel geo headers). Cleaned test rows. Also added IP Address to exports + get_useragent pipes (commit 512ee3488). |

| F15 | Bundle bloat in _app + shared lib/utils | FIXED (4 changes) | Lazy-loaded posthog-js (24MB, never configured) → only loads when key present. Moved hexToRgb out of lib/utils to annotate-document.ts → dropped pdf-lib (556KB) from shared client bundle. Moved bcrypt hashPassword/checkPassword to lib/auth/passwords.ts (server-only) → dropped bcryptjs from client. Made ai+openai imports type-only. Result: _app 449KB→211KB gz (-53%), /view/[linkId] ~1.09MB→692KB gz (-36%). |
