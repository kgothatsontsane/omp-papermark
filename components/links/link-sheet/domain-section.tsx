...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("dealroom.open-mic.co.za");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "dealroom.open-mic.co.za" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (isEditingCustomDomain && !canChangeCustomDomain) {
      setDisplayValue(data.domain ?? "dealroom.open-mic.co.za");
      return;
    }
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "dealroom.open-mic.co.za" });
      setDisplayValue("dealroom.open-mic.co.za");
      return;
    }

    // Check if this is a custom domain selection (not dealroom.open-mic.co.za)
    if (value !== "dealroom.open-mic.co.za") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "dealroom.open-mic.co.za")
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "dealroom.open-mic.co.za")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "dealroom.open-mic.co.za")
      : "dealroom.open-mic.co.za";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "dealroom.open-mic.co.za"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="dealroom.open-mic.co.za" className="hover:bg-muted">
              dealroom.open-mic.co.za
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "dealroom.open-mic.co.za" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "dealroom.open-mic.co.za" ? "flex" : "",
      ...
      {data.domain && data.domain !== "dealroom.open-mic.co.za" && !isDomainVerified ? (
